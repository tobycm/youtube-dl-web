...
import { download, Format, parseVideoURL, stringifyNumber, VideoMeta, VideoURL } from "../utils";
...
export default function DownloadPage({ prefill }: { prefill: string | null }) {
  ...
  async function queryMeta(id: string) {
    const meta: VideoMeta = await (await fetch(`${BACKEND_URL}/meta/${id}`)).json();
  ...
  async function onQueryClick() {
    ...
    setMetaLoading(true);
    try {
      const meta = await queryMeta(videoURL.id);
      setVideoMeta(meta);

      const subKeys = Object.keys(meta.subs);
      if (subKeys.length > 0) {
        if (subKeys.includes("en")) {
          // try to set format to english if english exists
          setTargetSubId("en");
        } else {
          // otherwise try to set sub format to first available sub
          setTargetSubId(subKeys[0]);
        }
      }
    } catch (error) {
      ...
      {videoMeta == null ? null : (
        <Box mt={5}>
          <Card variant="outlined" sx={{ mb: 3 }}>
            <CardMedia component="img" height="100px" image={videoMeta.thumbnail} />
            <CardHeader
              avatar={
                <Link color="inherit" href={`https://youtube.com/user/${videoMeta.author.name}`} underline="none">
                  <Avatar sx={{ bgcolor: theme.palette.primary.main }}>{videoMeta.author.name[0].toUpperCase()}</Avatar>
                </Link>
              }
              title={videoMeta.title}
              subheader={
                <Link color="inherit" href={`https://youtube.com/user/${videoMeta.author.name}`} underline="hover">
                  <strong>{videoMeta.author.name}</strong>
                </Link>
              }
            />
            <CardActions>
              <Box p={1} display="flex">
                ...
                <Tooltip arrow title={videoMeta.views.toLocaleString() + " views"}>
                  <Typography>{stringifyNumber(videoMeta.views)}</Typography>
                </Tooltip>
              </Box>
            </CardActions>
          </Card>
          ...
          <Accordion
            ...
            <AccordionDetails>
              {Object.keys(videoMeta.subs).length == 0 ? (
                <Alert severity="error">This video does not have any subtitle tracks.</Alert>
              ) : (
                <>
                  {shouldDownloadSubs && videoMeta.subs["live_chat"] != undefined ? (
                    <Alert severity="warning" sx={{ mb: 2 }}>
                      <AlertTitle>The live chat JSON file is quite large, and cannot be streamed.</AlertTitle>
                      It may take a long time for your download to start (around 15 seconds) so wait it out!
                    </Alert>
                  ) : shouldDownloadSubs && subFormat == "embed" ? (
                    <Alert severity="warning" sx={{ mb: 2 }}>
                      <AlertTitle>Embedding subtitles is still experimental</AlertTitle>
                      Download speeds may be slower due to subtitle injection!
                    </Alert>
                  ) : null}

                  <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
                    <Switch onChange={(e) => setShouldDownloadSubs(e.target.checked)} checked={shouldDownloadSubs} />
                    <Typography color={shouldDownloadSubs ? "text.primary" : "text.secondary"}>
                      {videoMeta.subs["live_chat"] != undefined
                        ? "Separately download live chat JSON for stream"
                    ...
                    {videoMeta.subs["live_chat"] != undefined ? null : (
                      <>
                        <Select
                          ...
                          {Object.keys(videoMeta.subs).map((subId, idx) => {
                            const name = videoMeta.subs[subId];
                            return <MenuItem value={subId}>{name}</MenuItem>;
                          })}
                        </Select>
                        <Typography color={shouldDownloadSubs ? "text.primary" : "text.secondary"}>and</Typography>
                        <Select
                          disabled={!shouldDownloadSubs}
                          value={subFormat}
                          onChange={(e) => {
            ...
            {videoMeta.formats.map((f: Format, i) => {
              ...
              return (
                <FormatCard
                  format={f}
                  type="use"
                  onClick={() => download(videoURL!.id, f, "none", shouldDownloadSubs, targetSubId, subFormat)}
                  key={f.id}
                  btnText="download"
                />
              );
          ...
          <Divider sx={{ mt: 3, mb: 3, color: theme.palette.text.secondary }}>or</Divider>
            ...
            <Box flexGrow={1}>
              ...
              {audioFormatFrom != null ? (
                audioFormatFrom == "none" ? (
                  <NoneDownloadCard title="No Audio" onClick={() => setAudioFormatFrom(null)} type="remove" />
                ) : (
                  <FormatCard format={audioFormatFrom} onClick={() => setAudioFormatFrom(null)} type="remove" btnText="" />
                )
              ) : (
                <Stack gap={1}>
                  <NoneDownloadCard title="No Audio" onClick={() => setAudioFormatFrom("none")} type="use" />
                  {videoMeta.formats.map((f: Format, i) => {
                    if (f.id.startsWith("sb")) return;
                    if (!f.audio) return;
                    if (f.video) return;

                    return <FormatCard format={f} onClick={() => setAudioFormatFrom(f)} type="use" key={f.id} btnText="USE" />;
                  })}
                </Stack>
              )}
            </Box>
            <Box flexGrow={1}>
              ...
              {videoFormatFrom != null ? (
                videoFormatFrom == "none" ? (
                  <NoneDownloadCard title="No Video" onClick={() => setVideoFormatFrom(null)} type="remove" />
                ) : (
                  <FormatCard format={videoFormatFrom} onClick={() => setVideoFormatFrom(null)} type="remove" btnText="" />
                )
              ) : (
                <Stack gap={1}>
                  <NoneDownloadCard title="No Video" onClick={() => setVideoFormatFrom("none")} type="use" />
                  {videoMeta.formats.map((f: Format, i) => {
                    if (f.id.startsWith("sb")) return;
                    if (!f.video) return;
                    if (f.audio) return;

                    return <FormatCard format={f} onClick={() => setVideoFormatFrom(f)} type="use" key={f.id} btnText="USE" />;
                  })}
                </Stack>
              )}
            </Box>
          ...
          {videoFormatFrom != null && audioFormatFrom != null && (
            <>
              <Tooltip
                arrow
                title={
                ...
                <LoadingButton
                  ...
                  size="large"
                  onClick={() => {
                    download(videoURL!.id, videoFormatFrom, audioFormatFrom, shouldDownloadSubs, targetSubId, subFormat);
                  }}>
                  Download Selected
                </LoadingButton>
              </Tooltip>
              <Typography color="text.secondary" mt={3} mb={1}>
                Note that downloading is done entirely using your browser's download system. If you do not see activity immediately, please wait a few
                seconds as your browser receives data.
              </Typography>
            </>
          )}
        </Box>
